...
export default function ListingsGrid({ listings: propListings = [], loading: propLoading, error: propError }) {
  const router = useRouter();
  
  // Access context (which will be available when wrapped in provider)
  const contextValue = useListings();
  const hasContext = contextValue !== null && contextValue !== undefined;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setHydrated(true);
    }, 100); // Increased timeout to ensure full hydration
    
    return () => clearTimeout(timer);
  }, []);

  // Prevent hydration mismatch by ensuring consistent rendering between server and client
  if (typeof window !== 'undefined' && !isClient) {
    // Return minimal skeleton during initial client render to prevent hydration mismatch
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {/* Minimal placeholder that matches server structure */}
      </div>
    );
  }
  ...
  return (
    ...
    >
      {safeListings.map((listing) => (
        <MemoizedListingCard
          key={listing._id || `listing-${Math.random().toString(36).substring(2, 9)}`}
          listing={listing}
        />
      ))}
    </div>
