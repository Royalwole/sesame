...
/**
 * Connect with retry logic using exponential backoff (internal function)
 *
 * Note: This is the internal implementation, use the exported version for external code
 */
async function _connectWithRetryInternal(uri, retryCount = 0) {
  try {
    ...
    if (retryCount < maxRetries) {
      ...
      await new Promise((resolve) => setTimeout(resolve, delay));
      return _connectWithRetryInternal(uri, retryCount + 1);
    }
...
export async function createIndexes() {
  try {
    console.log("[DB] Creating or verifying indexes...");

    // Create indexes for Listing model if it exists
    if (mongoose.models.Listing) {
      console.log("[DB] Creating indexes for Listing model");
    ...
    if (mongoose.models.User) {
      console.log("[DB] Creating indexes for User model");
    ...
    console.log("[DB] Indexes created or verified successfully");
  } catch (error) {
    console.warn("[DB] Error creating indexes:", error.message);
    // Non-fatal error - continue without indexes if needed
  }
}

/**
 * Enhanced connection with retry logic - uses existing connectDB function
 */
export async function connectWithRetry(maxRetries = 3) {
  ...
  while (attempts <= maxRetries) {
    try {
      ...
      // If requested via env var, create indexes
      ...
      console.warn(
        `[DB] Connection attempt ${attempts}/${maxRetries + 1} failed: ${error.message}`
      );
