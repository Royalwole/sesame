 ...
 * @returns {Promise<object>} - Result of role change
 */
export async function promoteToAgentPending(userId) {
...
export async function auditRoleChange(
  ...
  try {
    console.log(
      `Role change audit: User ${userId} from ${oldRole} to ${newRole} by admin ${adminId}. Reason: ${reason}`
    );

    // This would typically write to a database audit log table
