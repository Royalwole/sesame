...
export function validateListingQuery(query) {
  ...
  const numericParams = ['minPrice', 'maxPrice', 'bedrooms', 'bathrooms', 'page', 'limit'];
  ...
  stringParams.forEach(param => {
    if (query[param] && !/^[a-zA-Z0-9 ,\-]+$/i.test(query[param])) {
      throw new ValidationError(`Invalid ${param}: Contains invalid characters`);
    }
  });
...
export function validatePropertyInput(property) {
  ...
  if (!property.type || !['house', 'apartment', 'condo', 'townhouse', 'land'].includes(property.type)) {
