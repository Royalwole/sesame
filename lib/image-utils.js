...
/**
 * Utilities for handling images, including fallbacks between local and Blob storage
 */
import { isBlobConfigured } from "./blob";

// Base URL for Vercel Blob (this should be your project's Blob URL)
const VERCEL_BLOB_URL =
  process.env.NEXT_PUBLIC_VERCEL_BLOB_URL ||
  "https://public.blob.vercel-storage.com";

/**
 * Gets the appropriate image URL, checking Vercel Blob if the local image doesn't exist
 * @param {string} imagePath - The image path relative to /public
 * @param {string} fallbackPath - The fallback image path if neither local nor blob exists
...
export const getImageUrl = (
  ...
  // If Vercel Blob is configured, check there for the image
  if (isBlobConfigured() && !imagePath.includes("placeholder")) {
    // For non-placeholder images, try to load from Vercel Blob
    return `${VERCEL_BLOB_URL}/${cleanPath}`;
  }

  // Return the local path (Next.js will handle the 404 if not found)
  return `/${cleanPath}`;
};
 ...
 * @param {boolean} useSvgFallback - Whether to use SVG fallback (otherwise uses another JPG)
 */
export const handleImageError = (event, useSvgFallback = false) => {
  const imgElement = event.target;

  // Prevent infinite loop of error handling
  if (imgElement.getAttribute("data-error-handled") === "true") return;
  ...
  if (useSvgFallback) {
    // Use SVG fallback
    fallbackUrl = "/images/placeholder-property.svg";
  } else {
    // Use a different JPG fallback
    const isFirstFallback = imgElement.src.includes("placeholder-property.jpg");
    fallbackUrl = isFirstFallback
      ? "/images/placeholder-property-2.jpg"
      : "/images/placeholder-property.svg";
  }

  console.log(
    `Image failed to load: ${imgElement.src}, using fallback: ${fallbackUrl}`
  );

  // Apply the fallback image
  imgElement.src = fallbackUrl;

  // Add some styling to indicate this is a fallback
