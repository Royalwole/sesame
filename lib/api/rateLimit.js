...
export function createRateLimiter(
  options = { limit: 50, windowMs: 60 * 1000 }
) {
  // Create instance with configuration
  const rateLimiter = getRateLimitConfig({
    // Renamed from rateLimit to rateLimiter
  ...
  return async function checkRateLimit(req) {
    ...
    try {
      ...
      // Check rate limit
      const { success, limit, remaining, reset } = rateLimiter.check(
        token,
        options.limit,
        options.windowMs
      );
