...
class PermissionCache {
  constructor(options = {}) {
    ...
    this.domainCache = new Map(); // Cache config
    this.maxAge = options.maxAge || 120000; // 2 minutes default
  ...
  hasPermission(user, permission) {
    if (!this.enabled || !user || !permission) {
      const result = hasPermissionUncached(user, permission);
      console.log(
        `[Permission Check] Uncached check for ${permission}: ${result ? "GRANTED" : "DENIED"} (cache disabled or invalid params)`
      );
      return result;
    }
    ...
    if (!userKey) {
      const result = hasPermissionUncached(user, permission);
      console.log(
        `[Permission Check] Uncached check for ${permission}: ${result ? "GRANTED" : "DENIED"} (no user key)`
      );
      return result;
    }

    // Create a key for this specific permission check
    const permKey = `${userKey}:${permission}`; // Check if we have this specific result cached
    ...
    if (cachedResult && Date.now() < cachedResult.expiresAt) {
      ...
      console.log(
        `[Permission Check] Cache HIT for ${permission}: ${cachedResult.result ? "GRANTED" : "DENIED"} (user: ${userKey.split(":")[1]})`
      );
    ...
    console.log(
      `[Permission Check] Cache MISS for ${permission} (user: ${userKey.split(":")[1]})`
    );
    ...
    const userRole = user?.publicMetadata?.role || "unknown";
    const approved = user?.publicMetadata?.approved === true ? "yes" : "no";
    console.log(
      `[Permission Check] ${permission}: ${result ? "GRANTED" : "DENIED"} for role ${userRole} (approved: ${approved})`
    );
